...
// === FIX: Save payment method if coming from shipping.php ===
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mode_paiement'])) {
    $_SESSION['temp_mode_paiement'] = $_POST['mode_paiement']; // ← THIS LINE WAS MISSING
}

// Default to credit if not set
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_commande'])) {
    $livraison = new Livraison(
        ...
        $_SESSION['temp_lat'],
        $_SESSION['temp_lng'],
        $mode_paiement  // ← Now it's always a string
    );
