...
class TrackingService {
    ...
    public function simulateProgress(array $livraison, ?array $trajet = null): ?array {
        ...
        // Get or create route (ONLY ONCE) - MUST be real roads
        $route = null;
        $currentIndex = 0;

        if ($trajet && !empty($trajet['route_json'])) {
            // Use existing stored route
            $route = json_decode($trajet['route_json'], true);
            $currentIndex = (int)($trajet['current_index'] ?? 0);
        }

        // If no route, create it ONCE using OSRM (real roads only!)
        if (!$route || empty($route)) {
            ...
            if ($routeData && !empty($routeData['coordinates'])) {
                ...
                // NO FALLBACK - we MUST have real roads
        ...
        // FAST INCREMENT - move 3-5 points forward each update
        ...
        return [
            ...
            'has_arrived' => $hasArrived, // NEW: flag to indicate arrival
        ];
    }

    /**
     * Get route from OSRM - PRODUCTION READY with real roads
     */
    private function getOSRMRoute(array $origin, array $destination): ?array {
        ...
        $coordinates = [];
        // OSRM returns [lng, lat] pairs - convert to our format
        foreach ($data['routes'][0]['geometry']['coordinates'] as $point) {
            $coordinates[] = ['lat' => $point[1], 'lng' => $point[0]];
        }

        // Validate we got a reasonable route (at least 10 points for real roads)
    ...
    /**
     * Create linear route with many waypoints
     */
