...
class LivraisonController {
    ...
    private function creerCommande(int $idUtilisateur, array $postData): array {
        ...
        // Si le jeu demandé n'existe pas, on essaie d'en trouver un autre
        if (!$jeu) {
            $stmt = $this->db->query("SELECT * FROM jeu_jeu LIMIT 1");
            $jeu = $stmt->fetch();
        }

        // Si la table est vide, on crée un jeu de démo
        if (!$jeu) {
            try {
                // On tente une insertion avec les colonnes probables. 
                // Si ça échoue (colonnes différentes), on attrape l'erreur.
                ...
                // Fallback si la structure est différente (ex: pas de description ou src_img)
                // On essaie juste titre et prix
                try {
    ...
    private function insertTrajet(array $data): ?int {
        $stmt = $this->db->prepare('
            INSERT INTO trajet (
                ...
                statut_realtime, position_lat, position_lng
            ) VALUES (
                :id_livraison, :fournisseur_api, :identifiant_suivi,
                :statut_realtime, :position_lat, :position_lng
            )
