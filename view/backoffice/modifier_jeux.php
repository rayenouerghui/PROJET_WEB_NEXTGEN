...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($titre && $prix > 0 && $description && $id_categorie) {
        $jeu->setTitre($titre);
        $jeu->setPrix($prix);
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            if (in_array($ext, $allowed) && $file['size'] <= 5*1024*1024) {
                ...
                if ($jeu->getSrcImg()) @unlink('../../resources/' . $jeu->getSrcImg());
                $jeu->setSrcImg($filename);
            }
        }

        if (!empty($_FILES['video_src']['name'])) {
            ...
            if (pathinfo($file['name'], PATHINFO_EXTENSION) === 'mp4' && $file['size'] <= 100*1024*1024) {
                ...
                move_uploaded_file($file['tmp_name'], '../../resources/' . $filename);
                if ($jeu->getVideoSrc()) @unlink('../../resources/' . $jeu->getVideoSrc());
                $jeu->setVideoSrc($filename);
            }
        }
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-8">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              <input type="text" name="titre" value="<?= htmlspecialchars($jeu->getTitre()) ?>" required class="w-full px-5 py-4 bg-black/40 rounded-xl border border-purple-500 focus:border-cyan-400 text-white text-lg">
            </div>
            <div>
              <label class="block text-purple-300 font-bold mb-2">Prix (TND)</label>
              <input type="number" step="0.01" name="prix" value="<?= $jeu->getPrix() ?>" required class="w-full px-5 py-4 bg-black/40 rounded-xl border border-purple-500 focus:border-cyan-400 text-white text-lg">
            </div>
          </div>

          <div>
            <label class="block text-purple-300 font-bold mb-2">Description</label>
            <textarea name="description" rows="6" required class="w-full px-5 py-4 bg-black/40 rounded-xl border border-purple-500 focus:border-cyan-400 text-white text-lg"><?= htmlspecialchars($jeu->getDescription()) ?></textarea>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              <label class="block text-purple-300 font-bold mb-2">Image actuelle</label>
              <img src="../../resources/<?= htmlspecialchars($jeu->getSrcImg()) ?>" class="w-full max-w-sm rounded-xl ring-2 ring-purple-500">
              <label class="block text-purple-300 font-bold mt-4 mb-2">Nouvelle image</label>
              <input type="file" name="image" accept="image/*" class="w-full px-5 py-4 bg-black/40 rounded-xl border border-purple-500 text-white file:mr-4 file:py-3 file:px-6 file:rounded-xl file:border-0 file:bg-gradient-to-r file:from-purple-600 file:to-pink-600">
            </div>
            <div>
              <label class="block text-purple-300 font-bold mb-2">Vidéo actuelle</label>
              <?php if ($jeu->getVideoSrc()): ?>
                <video src="../../resources/<?= htmlspecialchars($jeu->getVideoSrc()) ?>" controls class="w-full max-w-sm rounded-xl ring-2 ring-purple-500"></video>
              <?php else: ?>
                <p class="text-gray-500 italic">Aucune vidéo</p>
              <?php endif; ?>
              <label class="block text-purple-300 font-bold mt-4 mb-2">Nouvelle vidéo (MP4)</label>
              <input type="file" name="video_src" accept="video/mp4" class="w-full px-5 py-4 bg-black/40 rounded-xl border border-purple-500 text-white file:mr-4 file:py-3 file:px-6 file:rounded-xl file:border-0 file:bg-gradient-to-r file:from-purple-600 file:to-pink-600">
            </div>
          </div>
