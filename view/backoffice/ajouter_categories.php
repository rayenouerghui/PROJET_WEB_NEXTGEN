...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nom && $desc) {
        $categorie = new Categorie($nom, $desc);
...
<head>
  ...
  <title>Ajouter une catégorie – NextGen Admin</title>
      ...
      <div class="glass rounded-2xl p-10">
        <form method="POST" class="space-y-8">
          <div>
            ...
            <input type="text" name="nom" value="<?= htmlspecialchars($_POST['nom'] ?? '') ?>" required class="w-full px-5 py-4 bg-black/40 rounded-xl border border-purple-500 focus:border-cyan-400 text-white text-lg">
          </div>
          <div>
          ...
          <div class="flex justify-center gap-6 pt-6">
            <button type="submit" class="btn-neon text-white font-bold py-4 px-12 rounded-xl text-xl hover:scale-105 transition">Ajouter la catégorie</button>
